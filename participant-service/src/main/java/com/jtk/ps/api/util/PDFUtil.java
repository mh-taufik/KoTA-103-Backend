...
public class PDFUtil {
    private PDFUtil() {
        throw new IllegalStateException("PDFUtil class");
    }
    ...
    public static float addParagraph(PDPageContentStream contentStream, float width, float sx,
                                     float sy, String text, float widthTab, boolean justify,
                                     boolean returnHeight) throws IOException {
        float height;
        ...
        for (String line : lines) {
            float charSpacing = 0;
            if (Boolean.TRUE.equals(justify)) {
                if (line.length() > 1) {
                    float size = FONT_SIZE * FONT.getStringWidth(line) / 1000;
                    float free = width - size;
                    if (firstLine) {
    ...
    public static float stringLength(int fontSize, String text) {
        try {
            return fontSize * PDType1Font.TIMES_ROMAN.getStringWidth(text) / 1000;
        } catch (IOException e) {
            return 0;
        }
    }

    public static float getNewLine(int fontSize) {
        return -1.07f * fontSize;
    }
}
